# CLAUDE.md

## Project Overview

Truss (`@ucsantacruz/truss`) is a web component library for UC Santa Cruz websites, built with **StencilJS** and **Sass**. Components use the web components standard for cross-browser compatibility and are published to npm.

## Tech Stack

- **Framework:** StencilJS 4.x (web components compiler)
- **Styling:** Sass (via `@stencil/sass` v3) using the `sass:color` and `sass:math` modules (no deprecated global builtins)
- **Language:** TypeScript (target ES2018, JSX factory `h`)
- **Testing:** Jest + Puppeteer (spec + e2e); Vitest 4 with `@vitest/browser-playwright` for Storybook story tests (via `@storybook/addon-vitest`)
- **Storybook:** Storybook 10.x (`@storybook/web-components-vite`)
- **Hosting:** Netlify (dev site)

## Project Structure

```
src/
  components/       # Web components (each in its own folder)
    trss-*/         # Component folders (tsx + scss + readme + tests)
  globals/          # Global styles and SCSS variables/mixins
    scss/
      variables.scss
      mixins.scss
    ucsc-trss.scss  # Global stylesheet
  utils/            # Shared utilities
  index.ts          # Entry point
  index.html        # Dev server page
  global.d.ts       # Ambient type declarations
```

## Key Commands

```bash
npm run dev        # Build with docs, watch, and serve (dev server)
npm run build      # Production build with docs
npm run test       # Run spec and e2e tests
npm run test.watch # Run tests in watch mode
npm run generate   # Scaffold a new component (interactive)
npm run storybook       # Run Stencil watch + Storybook dev server (port 6006)
npm run build-storybook # Build Storybook static site to storybook-static/
```

## Component Conventions

- All component tags are prefixed with `trss-` (e.g., `trss-alert`, `trss-ucsc-header`)
- Each component lives in `src/components/trss-<name>/` with:
  - `trss-<name>.tsx` — Component class using Stencil decorators (`@Component`, `@Prop`, `@State`, etc.)
  - `trss-<name>.scss` — Component styles
  - `readme.md` — Auto-generated docs (via `--docs` flag)
  - Optional test files
- Components use `scoped: false` (no shadow DOM) unless otherwise specified
- Props are documented with JSDoc comments (used for auto-generated docs)
- Slots are documented with `@slot` JSDoc tags

## Styling

- Global SCSS variables and mixins are auto-injected into all components via `injectGlobalPaths` in `stencil.config.ts`
- `@stencil/sass` v3 uses `@use ... as *` for injection (not `@import`), so any file that references variables from another injected file must `@use` it explicitly with a namespace (e.g., `_mixins.scss` uses `@use "variables" as v`)
- Use `color.adjust($color, $lightness: N%)` instead of `lighten()`/`darken()`, and `math.ceil()`/`math.floor()` instead of global `ceil()`/`floor()`
- The global stylesheet is at `src/globals/ucsc-trss.scss`
- The Stencil namespace is `ucsc-trss`

## Storybook

- Config lives in `.storybook/` — `main.ts`, `preview.ts`, `manager.ts`, `theme.ts`
- Stories are colocated with components as `trss-<name>.stories.tsx` (no separate stories directory)
- Component descriptions come automatically from the custom elements manifest (`custom-elements.json`) via `setCustomElementsManifest()` in `preview.ts` — no need to manually extract descriptions from readmes
- Roboto is loaded via Google Fonts in `manager-head.html` and `preview-head.html`
- The CEM is generated by a custom Stencil `docs-custom` output target (`stencil-to-cem.ts`)
- Story interaction tests run through Vitest in browser mode — see `vitest.config.ts` and `vitest.shims.d.ts` (the latter is auto-generated by `@storybook/addon-vitest` postinstall)

## Changelog

Uses [Conventional Commits](https://www.conventionalcommits.org/) and standard-version for versioning. Commit messages should follow the format:

```
type(scope): description
```

Common types: `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`

## Repository

- GitHub: https://github.com/ucsc/truss
- npm: https://www.npmjs.com/package/@ucsantacruz/truss
